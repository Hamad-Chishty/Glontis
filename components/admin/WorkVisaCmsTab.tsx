...
import { WorkVisaPageData } from '@/lib/types';
...
export default function WorkVisaCmsTab() {
  const { workVisaPage, setWorkVisaPage, updateData } = useData();
  const [formData, setFormData] = useState<WorkVisaPageData>(workVisaPage);
  const [saving, setSaving] = useState(false);
  const [notification, setNotification] = useState<string | null>(null);
  const [activeSectionTab, setActiveSectionTab] = useState<'general' | 'hero' | 'content' | 'countries' | 'jobs' | 'requirements' | 'process' | 'faqs' | 'seo'>('hero');

  const handleSave = async () => {
    setSaving(true);
    setWorkVisaPage(formData);
    const success = await updateData('UPDATE_ENTITY', 'workVisaPage', formData);
    setSaving(false);
    if (success) {
      setNotification('Work Visa CMS updated successfully!');
      setTimeout(() => setNotification(null), 3000);
    } else {
      setNotification('Failed to update Work Visa CMS');
      setTimeout(() => setNotification(null), 3000);
    }
  };

  // Section toggle helper
  const toggleVisibility = (key: keyof WorkVisaPageData['section_visibility']) => {
    setFormData((prev) => ({
      ...prev,
      section_visibility: {
  ...
  const addArrayString = (field: 'eligibility_requirements' | 'required_documents' | 'benefits') => {
    setFormData((prev) => ({
      ...prev,
      [field]: [...prev[field], 'New Item Entry'],
    }));
  };
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div className="space-y-1">
          ...
          <h2 className="text-xl sm:text-2xl font-black text-[#0A1838]">Work Visa Page Management</h2>
          <p className="text-xs text-slate-500 font-medium">
            Customize hero banners, job categories, eligible countries, requirements, and SEO settings for the Work Visa page.
          </p>
        </div>
      ...
      {/* Sub-Navigation Tabs for Admin Form */}
      <div className="flex items-center gap-1 overflow-x-auto pb-2 border-b border-slate-200 scrollbar-none">
        {[
          ...
          { id: 'countries', label: 'Countries & Destinations' },
          { id: 'jobs', label: 'Job Categories' },
          { id: 'requirements', label: 'Requirements & Documents' },
          { id: 'process', label: 'Process & Stats' },
      ...
      {activeSectionTab === 'content' && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
          <h3 className="text-base font-black text-[#0A1838] border-b border-slate-100 pb-3 flex items-center gap-2">
            <FileTextIcon className="w-4 h-4 text-[#F07100]" />
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-xs font-extrabold text-slate-700">Detailed Visa Overview</label>
              <textarea
                rows={5}
      ...
      {activeSectionTab === 'countries' && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <h3 className="text-base font-black text-[#0A1838] flex items-center gap-2">
              <GlobeIcon className="w-4 h-4 text-[#F07100]" />
              <span>Available Work Visa Destinations ({formData.countries_available.length})</span>
            </h3>
            ...
            <button
              onClick={() => {
                const newItem = {
                  ...
                  badge: 'Work Permit',
                  description: 'Description of work visa opportunities in this country.',
                  points: ['Point 1', 'Point 2'],
                  image_url: 'https://images.unsplash.com/photo-1513635269975-59663e0ac1ad?auto=format&fit=crop&w=800&q=80',
                  is_active: true,
                };
                setFormData({ ...formData, countries_available: [...formData.countries_available, newItem] });
              }}
          ...
          <div className="space-y-6">
            {formData.countries_available.map((item, idx) => (
              <div key={item.id || idx} className="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() => {
                      setFormData({
                        ...formData,
                        countries_available: formData.countries_available.filter((_, i) => i !== idx),
                      });
                    }}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[11px] font-bold text-slate-700">Country Name</label>
                    <input
                      type="text"
                      value={item.name}
                      onChange={(e) => {
                        const updated = [...formData.countries_available];
                        updated[idx].name = e.target.value;
                        setFormData({ ...formData, countries_available: updated });
                      }}
                  ...
                  <div className="md:col-span-2 space-y-1">
                    <label className="text-[11px] font-bold text-slate-700">Key Feature Points (Comma Separated)</label>
                    <input
                      type="text"
                      value={item.points ? item.points.join(', ') : ''}
                      onChange={(e) => {
                        const updated = [...formData.countries_available];
                        updated[idx].points = e.target.value.split(',').map((s) => s.trim()).filter(Boolean);
                        setFormData({ ...formData, countries_available: updated });
                      }}
                      className="w-full px-3 py-2 text-xs rounded-lg border border-slate-300"
                    />
      ...
      {activeSectionTab === 'jobs' && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <h3 className="text-base font-black text-[#0A1838] flex items-center gap-2">
              <BriefcaseIcon className="w-4 h-4 text-[#F07100]" />
              <span>In-Demand Job Categories ({formData.job_categories.length})</span>
            </h3>
            ...
            <button
              onClick={() => {
                const newItem = {
                  id: `jc-${Date.now()}`,
                  title: 'New Job Industry',
                  badge: 'High Demand',
                  description: 'Overview of roles and vacancies in this sector.',
                  key_requirements: ['Degree or Diploma', 'Experience'],
                  image_url: 'https://images.unsplash.com/photo-1521737711867-e3b97375f902?auto=format&fit=crop&w=800&q=80',
                  is_active: true,
                };
                setFormData({ ...formData, job_categories: [...formData.job_categories, newItem] });
              }}
              className="px-3.5 py-1.5 rounded-xl bg-slate-900 hover:bg-[#F07100] text-white text-xs font-bold flex items-center gap-1.5 transition-colors"
            >
              <PlusIcon className="w-3.5 h-3.5" />
              <span>Add Job Sector</span>
            </button>
          </div>

          <div className="space-y-6">
            {formData.job_categories.map((item, idx) => (
              <div key={item.id || idx} className="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-black uppercase text-[#F07100]">Category #{idx + 1}</span>
                  <button
                    onClick={() => {
                      setFormData({
                        ...formData,
                        job_categories: formData.job_categories.filter((_, i) => i !== idx),
                      });
                    }}
                    className="p-1.5 text-rose-600 hover:bg-rose-100 rounded-lg transition-colors"
                  >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[11px] font-bold text-slate-700">Category Title</label>
                    <input
                      type="text"
                      value={item.title}
                      onChange={(e) => {
                        const updated = [...formData.job_categories];
                        updated[idx].title = e.target.value;
                        setFormData({ ...formData, job_categories: updated });
                      }}
                      className="w-full px-3 py-2 text-xs rounded-lg border border-slate-300"
                    />
                  </div>
                  ...
                  <div className="md:col-span-2 space-y-1">
                    <label className="text-[11px] font-bold text-slate-700">Key Requirements (Comma Separated)</label>
                    <input
                      type="text"
                      value={item.key_requirements ? item.key_requirements.join(', ') : ''}
                      onChange={(e) => {
                        const updated = [...formData.job_categories];
                        updated[idx].key_requirements = e.target.value.split(',').map((s) => s.trim()).filter(Boolean);
                        setFormData({ ...formData, job_categories: updated });
                      }}
                      className="w-full px-3 py-2 text-xs rounded-lg border border-slate-300"
                    />
      ...
      {activeSectionTab === 'process' && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
          <h3 className="text-base font-black text-[#0A1838] border-b border-slate-100 pb-3 flex items-center gap-2">
            <ClockIcon className="w-4 h-4 text-[#F07100]" />
            <span>Processing Summary & Key Stats</span>
          </h3>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-700">Average Processing Time</label>
              <input
                type="text"
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-700">Visa Duration & Validity</label>
              <input
                type="text"
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    processing_information: { ...formData.processing_information, visa_duration: e.target.value },
                  })
                }
                className="w-full px-3 py-2 text-xs rounded-xl border border-slate-300"
              />
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-700">Work Rights</label>
              <input
                type="text"
                value={formData.processing_information.work_rights}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    processing_information: { ...formData.processing_information, work_rights: e.target.value },
                  })
                }
                className="w-full px-3 py-2 text-xs rounded-xl border border-slate-300"
              />
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-700">Family Dependents Rights</label>
              <input
                type="text"
                value={formData.processing_information.family_dependents}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    processing_information: { ...formData.processing_information, family_dependents: e.target.value },
                  })
                }
                className="w-full px-3 py-2 text-xs rounded-xl border border-slate-300"
              />
      ...
      {activeSectionTab === 'faqs' && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <h3 className="text-base font-black text-[#0A1838] flex items-center gap-2">
              <HelpCircleIcon className="w-4 h-4 text-[#F07100]" />
              <span>Work Visa FAQs ({formData.faqs.length})</span>
            </h3>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {Object.entries(formData.section_visibility).map(([key, isVisible]) => (
                ...
                >
                  ...
                  {isVisible ? <EyeIcon className="w-3.5 h-3.5 text-emerald-600" /> : <EyeOffIcon className="w-3.5 h-3.5 text-slate-400" />}
                </button>
              ))}
            </div>
          </div>
